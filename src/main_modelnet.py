...
def load_data(config):
    ...
    transform = T.Compose([
        T.SamplePoints(config["num_points"]),        # mesh -> point cloud (pos: [N,3])
        T.NormalizeScale(),                          # center + scale to unit sphere
    ])

    # Load ModelNet10 dataset
    train_dataset = ModelNet(root=config["data_path"], name='10', train=True, transform=transform)
    test_dataset = ModelNet(root=config["data_path"], name='10', train=False, transform=transform)
...
def visualize_examples(dataset, class_names, config, sample_idx=10):
    ...
    print(f"Visualizing a single point cloud from the training set (sampled {config['num_points']} points)...")

    sample = dataset[sample_idx]
    visualize_point_cloud_matplotlib(
        sample.pos.numpy(),
        label=f'{class_names[sample.y.item()]} (sampled {config["num_points"]} points)',
        figsize=(10, 8),
        output_path=config["vis_single_example_path"]
    )

    # Visualize one example from each class
    print("Creating visualization grid for all classes (with transformations)...")

    # Find one example from each class in the transformed dataset
    class_examples = {}
    ...
    visualize_multiple_point_clouds(
        ...
        title='ModelNet10 Dataset - Example from Each Class (transformed)'
    )
